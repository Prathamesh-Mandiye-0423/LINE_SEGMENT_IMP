...
def candidate_squares(segments: List[LineSegment],
                     blue_segments: List[LineSegment]) -> List[Square]:
    if not segments:
        return []
    ...
    if width > height:
        # Width is larger, so we fix x and vary y
        fixed_x_min = x_min
        blue_y_coords = set()
        ...
        # Height is larger, so we fix y and vary x
        fixed_y_min = y_min
        blue_x_coords = set()
    ...
    # If width == height or no candidates found, add the default square
    if not candidates:
        square = Square(x_min, y_min, side_length)
        blue_count = count_segments_in_square(blue_segments, square)
        square.blue_count = blue_count
...
def find_optimal_disjoint_pair(A1: List[Square], A2: List[Square]) -> Tuple[Optional[Square], Optional[Square], int]:
    if not A1 or not A2:
        return None, None, float('inf')

    # 1. HASH MAP OPTIMIZATION: Store A2 squares by their PST result key
    # Key: (x_min, y_min, blue_count) -> Value: Square Object
    # This allows O(1) lookup after the O(log k) PST query.
    a2_lookup = { (s.x_min, s.y_min, s.blue_count): s for s in A2 }

    # 2. Build PST on A2 points
    ...
    for s1 in A1:
        # Case 1: s2 is to the right of s1
        res_r = pst.minquery(s1.x_max + 1e-9, -float('inf'))
        
        # Case 2: s2 is above s1
        res_a = pst.minquery(-float('inf'), s1.y_max + 1e-9)
        
        for result in [res_r, res_a]:
            if result:
                # O(1) Lookup using the hash map
                matching_s2 = a2_lookup.get(result)
                
                if matching_s2 and s1.disjointCheck(matching_s2):
                    total = s1.blue_count + matching_s2.blue_count
                    if total < min_total:
                        min_total = total
                        best_s1, best_s2 = s1, matching_s2
                        
    return best_s1, best_s2, min_total

def horizontal_sweep_squares(red_segments: List[LineSegment], blue_segments: List[LineSegment], valid_y_positions: List[float], use_pst: bool = True) -> Tuple[Square, Square, int]:
    min_blue_count = float('inf')
    best_s1 = None
    best_s2 = None
    for y_pos in valid_y_positions:
        Q1 = [seg for seg in red_segments if (seg.y_min() + seg.y_max()) / 2 <= y_pos]
        Q2 = [seg for seg in red_segments if (seg.y_min() + seg.y_max()) / 2 > y_pos]
        if not Q1 or not Q2:
            continue
...
def vertical_sweep_squares(red_segments: List[LineSegment], blue_segments:List[LineSegment],
    ...
    for x_pos in valid_x_positions:
        Q1 = [seg for seg in red_segments if (seg.x_min() + seg.x_max()) / 2 <= x_pos]
        Q2 = [seg for seg in red_segments if (seg.x_min() + seg.x_max()) / 2 > x_pos]
        if not Q1 or not Q2:
            continue
